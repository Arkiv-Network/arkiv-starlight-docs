...
<Aside type="danger">
Without a project attribute, your queries may return data from other projects, and other projects may see yours.
</Aside>
...
The same applies to the field selection: `select()` fetches only the fields you name, so ask for what you'll actually use. `select()` with no argument (fetch everything) is convenient while prototyping but wasteful in production.
...
Numeric attribute values **must be integers** — a float like `19.99` throws an `InvalidAttributeError`. Scale non-integers to a fixed precision instead:

```ts
// Good: scaled to cents — integer, still supports range queries
{ key: 'priceCents', value: 1999 }

// Throws InvalidAttributeError
...
## 16. Remember That updateEntity Is a Full Replace

`updateEntity` is a full replace, not a patch: any attribute you don't re-send is removed, and the payload, content type, and expiration are overwritten. To change only some fields, read the entity first and re-send the complete state with your changes applied:
...
See [Mutating Data](/typescript-sdk/mutating-data/#update-entity) for details.
