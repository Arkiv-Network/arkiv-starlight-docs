...
Combining both patterns in a single component:
...
function PostManager() {
  ...
  async function handleCreate() {
    if (!wagmiWalletClient || !address) return
    ...
    await arkivClient.createEntity({
      payload: jsonToPayload({ title: "New Post", content: "Written from React!" }),
      contentType: "application/json",
      attributes: [
        { key: "project", value: "my-app" },
        { key: "entityType", value: "post" },
    ...
    queryClient.invalidateQueries({ queryKey: ["arkiv", "entities", "post"] })
  }
  ...
  return (
    <div>
      <button onClick={handleCreate} disabled={!wagmiWalletClient}>
        Create Post
      </button>
      <ul>
        {posts?.map((post) => (
