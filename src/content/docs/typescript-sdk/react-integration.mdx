...
<Aside type="caution">
Do not use `useEffect` and `useState` for data fetching. This combination does not handle caching, race conditions, deduplication, or background refetching. Always use a data-fetching library like TanStack Query or SWR.
</Aside>

## Writing Data with wagmi

Your project can use [wagmi](https://wagmi.sh), or a wagmi-powered kit such as RainbowKit or ConnectKit. You can then derive an Arkiv wallet client directly from the wagmi wallet client. This avoids duplicate wallet connection logic.
...
<Aside type="tip">
The `transport: custom(wagmiWalletClient.transport)` line is the key. It reuses the wallet connection that wagmi already manages. Users then do not see a second connect prompt.
</Aside>
...
If you do not use wagmi, you can connect MetaMask directly:
...
// Prompt MetaMask to add the network (no effect if it is already added)
...
<Aside>
For a complete browser example of this approach, see the [MetaMask Sketch App](/learn/metamask-sketch-app/) tutorial.
</Aside>
