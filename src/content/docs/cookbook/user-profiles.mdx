...
- an [access key](/start-here/access-keys/) for the Tiramisu RPC endpoint
...
1. **Set up the clients**
   ...
   <Aside type="tip">
   Anonymous access to the RPC endpoint is rate limited. Register a project on the [access keys page](/start-here/access-keys/) and pass the key in the RPC URL, as a `X-API-KEY` header, or as a bearer token.
   </Aside>
