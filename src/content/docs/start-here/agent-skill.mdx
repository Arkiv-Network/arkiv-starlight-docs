...
- 14 best practices (project attributes, security, data modeling, error handling)
- Integration patterns (backend, React + TanStack Query, wagmi)
...
- *"Add a query that filters entities by owner and sorts by creation date"*
- *"Set up a React hook that reads Arkiv entities with TanStack Query"*

The agent will use the skill's knowledge to generate accurate, idiomatic code — including things like project attributes, proper expiration handling, and batch operations — without you needing to explain the API.
