...
Every entity has two system-level ownership fields that are distinct from any custom attributes you define:

- **`$owner`** — The wallet address that currently controls the entity (can update, delete, extend, or transfer it).
- **`$creator`** — The wallet that originally created the entity. Immutable.

Both are queryable like any other attribute — `eq('$owner', address)` is equivalent to the `.ownedBy(address)` shorthand, and `eq('$creator', address)` to `.createdBy(address)`.

If your app needs an identity field that is *not* the writing wallet (e.g. a backend creates entities on behalf of users), add a custom attribute — and give it a domain-specific name like `author` or `profileId` rather than `owner`, which is too easy to confuse with the system-level `$owner`:

```ts
attributes: [
  { key: 'author', value: userAddress.toLowerCase() },
  { key: 'type', value: 'note' },
]
```

See the [best-practices guide](/typescript-sdk/best-practices/) for the filter-by-creator and relational patterns.
