...
<Aside type="caution">
Numeric attribute values must be **integers** — a float like `1.5` throws an `InvalidAttributeError`. Scale non-integers to a fixed precision instead (e.g. store `1.5` as `1500` and divide on read).
</Aside>
...
<Aside type="caution">
Arkiv measures expiration in whole blocks (1 block = 2 seconds), so `expiresIn` must be a **positive integer and a multiple of 2** — otherwise the SDK throws an `InvalidExpirationError`.
</Aside>
...
The SDK's query builder supports more than basic filtering. Before you reach for manual workarounds, check what's built in:

- **Field selection** — fetch only the entity fields you need with `select({ key: true, payload: true })`; the result type is narrowed to exactly those fields
