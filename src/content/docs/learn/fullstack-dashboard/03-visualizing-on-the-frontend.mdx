...
With the backend now publishing data, build the frontend that consumes it. This step uses a public client from the Arkiv SDK, which reads data without a private key.
...
<Aside>
Cheesecake has no public block explorer yet. Each price box shows a short entity key instead of an explorer link.
</Aside>
...
import { cheesecake } from 'https://esm.sh/@arkiv-network/sdk/chains?target=es2022&bundle-deps';
