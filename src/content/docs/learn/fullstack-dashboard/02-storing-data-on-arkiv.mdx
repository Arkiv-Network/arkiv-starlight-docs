...
const client = createWalletClient({
    chain: braga, // We are using the Braga testnet.
    transport: http(),
    account: account,
});
...
<Aside type="caution">
The Braga faucet and explorer went offline when Braga was retired on 12 August 2026, so this step is blocked until the new testnet ships in September 2026. See the [Braga page](/networks/braga/).
</Aside>

Before we can create entities on Arkiv, we need some test GLM to pay for gas fees. Braga uses GLM as its native gas token. Visit the Braga faucet:

**👉 [https://braga.hoodi.arkiv.network/faucet/](https://braga.hoodi.arkiv.network/faucet/)**

Enter your wallet address (the one displayed in the previous step) and request test GLM. This is completely free and only takes a moment.

You may need to wait a few minutes for the transaction to be processed. You can check your balance using a Braga block explorer: **👉 [https://explorer.braga.hoodi.arkiv.network/](https://explorer.braga.hoodi.arkiv.network/)**
...
<Aside type="caution">
Running the backend means sending transactions to Braga, which was retired on 12 August 2026. There's no public network until the new testnet ships in September 2026. See the [Braga page](/networks/braga/).
</Aside>
