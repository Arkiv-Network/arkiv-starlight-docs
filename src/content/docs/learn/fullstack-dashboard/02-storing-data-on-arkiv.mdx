...
const client = createWalletClient({
    chain: cheesecake, // Use the Cheesecake testnet.
    transport: http(),
    account: account,
});
...
Before you create entities on Arkiv, you need test GLM to pay for gas fees. Cheesecake uses GLM as its native gas token.

Cheesecake has no public faucet yet. Ask in the [Arkiv Discord](https://discord.gg/arkiv), and a team member sends test GLM to the address that the previous step printed.
...
This is the core of the backend. This function takes all the crypto data and creates entities on Arkiv in one batch operation with `executeBatch`.
...
- **expires**: entities can expire automatically after a set time.
...
The logs show your address and each entity it creates. Leave the script running. In the next section, you build a frontend to see the data.
